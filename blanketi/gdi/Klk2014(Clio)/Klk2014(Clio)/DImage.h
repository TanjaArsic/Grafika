...
public:
	...
	// Vraca pointer na prvi bajt sa pikselima
	unsigned char* GetDIBBits() { return m_pBuf; }
	// Pozvati nakon direktne izmene bafera.
	void	Update();

	void Brightness(int value);
	void Rotate(CDC* pDC, int angle);

	void DrawTransparent(CDC* pDC, CRect rcImg, CRect rcDC, COLORREF bgColor);

protected:
	CPoint Izracunaj(float angle, CPoint Axis, CPoint point);
	void Flip();
